...
- **Twitch login auto-creates / re-activates a Streamer entity.** Confirmed in `apps/server/src/auth/persistOAuthUser.ts:persistTwitchOAuthUser`: every Twitch OAuth callback upserts a `Streamer` row by `twitchId` (or matching `username`/`name`), sets `ownerId = user.id`, `isActive = true`, and upserts a `StreamerMember` with role `'OWNER'`. The derived `STREAMER` system role then comes from `resolveUserStreamerContext` in `apps/server/src/auth/resolvePrismaUserFromSession.ts` (finds any active `Streamer` where the user is `ownerId` OR an `OWNER` member). Google / Apple / email logins do NOT auto-create a Streamer. `DELETE /api/admin/streamers/:id` only flips `isActive = false`; the next Twitch login re-activates it because `persistTwitchOAuthUser` writes `isActive: true` unconditionally — there is no persistent admin-suppression flag today. Treat this as intentional product behavior; persistent suppression requires a schema migration + product/legal decision and is out of scope for code-only changes.
