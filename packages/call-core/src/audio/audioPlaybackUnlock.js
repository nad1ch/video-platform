...
export function registerAudioUnlockHook(fn) {
    unlockHooks.add(fn);
    return () => {
        unlockHooks.delete(fn);
    };
}
export function playAllPageAudio() {
...
let lastPlayAllTs = 0;
export function playAllPageAudioThrottled() {
    const now = Date.now();
...
export function initAudioPlaybackUnlock() {
