 ...
 */
export function loadCallNoiseSuppressionPreference() {
 ...
 */
export function saveCallNoiseSuppressionPreference(enabled) {
    if (typeof localStorage === 'undefined')
        return;
    try {
 ...
 */
export function mergeCallOutgoingMicAudioCaptureConstraints(base) {
    return { ...base, echoCancellation: true, noiseSuppression: true, autoGainControl: true };
}
function warnIfNoiseSuppressionMismatch(track) {
 ...
 * bundle is rejected.
 */
export async function applyOutgoingCallMicCaptureConstraints(track) {
