...
export function aggregateInboundVideoStatsByPeerId(rows) {
    const acc = new Map();
    for (const r of rows) {
    ...
    for (const [id, a] of acc) {
        out.set(id, {
            ...
            framesPerSecond: a.fpsSamples > 0 && Number.isFinite(a.minFps) ? a.minFps : undefined,
        });
    }
    return out;
}
export function evaluateInboundFpsRenderPressure(stats) {
    if (!stats) {
        return 'unknown';
    }
...
export function applyFpsRenderPressure(baseFps, pressure) {
