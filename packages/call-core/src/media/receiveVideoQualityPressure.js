 ...
 */
export function peerRecvLayerCategory(peerId, input) {
 ...
 */
export function applyReceiveQualityPressureToLayers(base, pressure, input) {
    void pressure;
    void input;
    return new Map(base);
}
 ...
 */
export function evaluateInboundVideoStatsForPressure(rows, previousPacketsLostSum) {
    const videoRows = rows.filter((r) => (typeof r.framesDecoded === 'number' && r.framesDecoded > 0) ||
    ...
    if (previousPacketsLostSum !== null &&
    ...
    const lossDelta = previousPacketsLostSum === null ? 0 : Math.max(0, packetsLostSum - previousPacketsLostSum);
