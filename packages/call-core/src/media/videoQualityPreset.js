...
export const VIDEO_QUALITY_PRESETS = ['economy', 'balanced', 'hd'];
...
export function isVideoQualityPreset(v) {
    return v === 'economy' || v === 'balanced' || v === 'hd';
}
function clampCallVideoFramerate(fps) {
    return Math.min(CALL_VIDEO_MAX_FRAMERATE, Math.max(CALL_VIDEO_MIN_FRAMERATE, Math.round(fps)));
}
export function clampCallVideoBitrate(bps) {
    return Math.min(CALL_VIDEO_MAX_BITRATE_BPS, Math.max(CALL_VIDEO_MIN_BITRATE_BPS, Math.round(bps)));
}
export function clampCallVideoBitrateSmallRoom(bps) {
    return Math.min(CALL_VIDEO_MAX_BITRATE_BPS_SMALL_ROOM, Math.max(CALL_VIDEO_MIN_BITRATE_BPS_SMALL_ROOM, Math.round(bps)));
}
export const FIXED_CALL_VIDEO_ENCODING = {
 ...
 */
export function countActiveCameraPublishersAtWire(existingProducers, selfPeerId, selfWillPublishVideo) {
    const ids = new Set();
 ...
 */
export function resolveOutgoingVideoPublishTier(input) {
...
export function resolveVideoPublishTier(preset, explicit) {
    void preset;
    void explicit;
    return 'auto_large_room';
}
export const VIDEO_PRESET_MOBILE = {
...
export function getCallVideoConstraints(tier) {
    void tier;
 ...
 */
export function getCallVideoConstraintsForRuntime(tier, isMobile) {
    if (isMobile) {
 ...
 */
export function getSingleLayerEncodingsForPreset(tier) {
...
export function getSimulcastEncodingsForPreset(tier) {
 ...
 */
export function getDesktopSimulcastEncodingsForLargeRoom() {
    return [
        {
            rid: 'q',
 ...
 */
export function shouldUsePublisherSimulcast(input) {
    if (!input.enabled) {
        return false;
    }
    if (input.isMobile) {
 ...
 */
export function getOutgoingVideoEncodings(tier, simulcast) {
    if (simulcast) {
