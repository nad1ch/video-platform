...
import { applyOutgoingCallMicCaptureConstraints, mergeCallOutgoingMicAudioCaptureConstraints, } from '../audio/noiseSuppressionPreference';
import { persistVideoInputDeviceIdFromTrack, readPreferredVideoInputDeviceId, selectPreferredVideoInputDeviceId, } from './preferredVideoInputDevice';
...
async function getUserMediaWithDeviceIdExactThenIdeal(constraints, kind) {
...
export function useLocalMedia(options) {
    const resolveTier = () => options?.getVideoPublishTier?.() ?? 'auto_large_room';
    const resolveMediaMode = () => options?.mediaMode?.() === 'audio-only' ? 'audio-only' : 'audio-video';
    ...
    function mapAudioOutputs(list) {
        return list
            .filter((d) => d.kind === 'audiooutput')
            .map((d) => ({
            deviceId: d.deviceId,
            label: typeof d.label === 'string' && d.label.trim().length > 0 ? d.label.trim() : 'Speaker',
        }));
    }
     ...
     */
    function attachLocalTrackEndedListener(track) {
        const onEnded = () => {
            const stream = localStream.value;
            if (!stream) {
                return;
    ...
    async function startLocalMedia() {
        ...
        const requestedAudio = mergeCallOutgoingMicAudioCaptureConstraints(DEFAULT_CALL_AUDIO_CONSTRAINTS);
        const stream = preferred !== undefined
     ...
     */
    async function swapLocalAudioInput(deviceId) {
        const stream = localStream.value;
        if (!stream) {
     ...
     */
    async function swapLocalVideoInput(deviceId) {
        const stream = localStream.value;
        if (!stream) {
            throw new Error('Local stream not started');
        }
