 ...
 */
function localCameraPreviewStream(localStream) {
    if (!localStream) {
        return null;
    }
    const videos = localStream.getVideoTracks();
    const audios = typeof localStream.getAudioTracks === 'function' ? localStream.getAudioTracks() : [];
    if (videos.length === 0) {
        return localStream;
    }
...
export function localPreviewStreamForOutbound(outboundVideoSource, screenShareStream, localStream) {
