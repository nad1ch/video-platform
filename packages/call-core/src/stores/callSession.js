...
import { ref } from 'vue';
import { isVideoQualityPreset } from '../media/videoQualityPreset';
...
function readVideoQualityFromStorage() {
...
export const useCallSessionStore = defineStore('callSession', () => {
    ...
    const videoQualityPreset = ref(readVideoQualityFromStorage().preset);
    ...
    function mergeRemotePeersFromProducerSync(peers) {
        if (peers.length === 0) {
            return;
        }
    ...
    function upsertRemoteDisplayName(peerId, displayName, avatarUrl) {
    ...
    function setVideoQualityPreset(p) {
    ...
    function setVideoQualityImplicitDefault() {
    ...
    function setCallDebugOverlay(v) {
    ...
    function setSignalingAuthUserId(id) {
