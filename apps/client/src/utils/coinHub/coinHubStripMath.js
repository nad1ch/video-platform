 ...
 */
import { buildSpinStripFromPayout, } from '@/utils/coinHub/coinHubSpinReel';
export { getSpinRarity, getSpinRarityForLabel, } from '@/utils/coinHub/coinHubRarity';
export { buildSpinStripFromPayout };
export function endTranslateX(viewportWidthPx, itemWidthPx, landIndex) {
    if (!Number.isFinite(viewportWidthPx) || !Number.isFinite(itemWidthPx) || viewportWidthPx <= 0) {
        return 0;
    }
    if (itemWidthPx <= 0)
        return 0;
    if (!Number.isFinite(landIndex) || landIndex < 0)
        return 0;
...
export const SLOT_VISUAL_POOL = [5, 8, 12, 20, 25, 50, 100];
const WEIGHTED_POOL = [
    { value: 5, weight: 30 },
    { value: 8, weight: 25 },
    { value: 12, weight: 20 },
...
export function pickWeightedVisualReward(rng = Math.random) {
...
export function mapSpinScrollProgress(u) {
    return mapSlotMachineProgress(u);
}
const CASE_POOL = ['+5', '+10', '+15', '+20', '+25', '+30', '+50', '+100'];
export function buildCaseStripCells(targetLine) {
    const out = [];
