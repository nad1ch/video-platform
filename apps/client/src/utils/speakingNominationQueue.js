 ...
 */
export function decodeSpeakingNominationFlat(flat) {
...
export function seatsInvolvedInSpeakingNominationFlat(flat) {
    const s = new Set();
    if (!Array.isArray(flat) || flat.length < 1) {
        return s;
    }
 ...
 */
export function nominationTargetSeatsFromSpeakingFlat(flat) {
    const s = new Set();
    if (!Array.isArray(flat) || flat.length < 1) {
        return s;
    }
