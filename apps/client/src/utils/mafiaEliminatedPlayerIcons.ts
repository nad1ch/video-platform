...
function hashTextToUint(value: string): number {
  let h = 2166136261
...
function cleanPeerIds(peerIds: readonly string[]): string[] {
  const seen = new Set<string>()
  const out: string[] = []
  for (const peerId of peerIds) {
    const id = typeof peerId === 'string' ? peerId.trim() : ''
    if (id.length < 1 || seen.has(id)) continue
    seen.add(id)
    out.push(id)
  }
  return out
...
export function mafiaEliminatedPlayerIconForPeerId(peerId: string, seed = ''): string {
...
export function mafiaEliminatedPlayerIconMapForPeerIds(peerIds: readonly string[], seed = ''): Map<string, string> {
