 ...
 */
export const LANDING_HASH_SCROLL_TOP_PX = {
    '#videocall': 721,
    '#games': 1139,
    '#economy': 1735,
    '#footer': 2204,
...
export function getLandingScrollTopForHash(hash) {
    ...
    if (typeof window === 'undefined')
        return designTop;
    const canvasWidth = document.querySelector('.landing__canvas')?.getBoundingClientRect().width ??
        Math.min(window.innerWidth, LANDING_CANVAS_DESIGN_WIDTH_PX);
    const scale = canvasWidth > 0 ? Math.min(canvasWidth / LANDING_CANVAS_DESIGN_WIDTH_PX, 1) : 1;
