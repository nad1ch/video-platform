...
const GAME_ROUTE_NAMES = new Map([
...
const ROUTE_TO_DIAGNOSTIC_GAME_TYPE = new Map([
    ['mafia', 'mafia'],
 ...
 */
function deriveDiagnosticsRoomId(route) {
...
function applyDiagnosticsContextForRoute(route) {
    try {
...
export function redactSensitiveDiagnosticsText(value) {
    return redactSensitiveText(value);
}
export function sanitizeDiagnosticsMetadata(input) {
    return sanitizeMetadata(input);
}
export function trackClientEvent(event, metadata, path = currentPath()) {
    const now = Date.now();
...
export function trackClientError(error, metadata, source = 'client', path = currentPath()) {
...
export function initClientAnalytics(router) {
