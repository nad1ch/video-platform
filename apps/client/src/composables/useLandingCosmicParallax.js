...
function landingPageScrollY() {
    if (typeof window === 'undefined')
        return 0;
    return (document.scrollingElement?.scrollTop ?? window.pageYOffset ?? document.documentElement?.scrollTop ?? 0);
}
export function useLandingCosmicParallax(canvasEl) {
    let cleanup;
    onMounted(() => {
        if (typeof window === 'undefined')
            return;
