...
import type {
  GameRoomPlayerKickPayload,
  GameRoomPlayerLifeState,
  GameRoomPlayerLifeStateSnapshotPayload,
  GameRoomPlayerRevivePayload,
  GameRoomPlayersUpdatePayload,
  GameRoomReshufflePayload,
  GameRoomTimerStartPayload,
  GameRoomTimerStopPayload,
...
function normalizeGameRoomSpeakingQueueFlat(raw: unknown): number[] {
...
function parseGameRoomHostUpdated(
  data: unknown,
  ...
  if (o.type !== GameRoomWs.hostUpdated) return null
  const p = o.payload
...
function parseGameRoomQueueUpdate(data: unknown): { speakingQueue: number[] } | null {
  if (!data || typeof data !== 'object') return null
  const o = data as { type?: unknown; payload?: unknown }
  if (o.type !== GameRoomWs.queueUpdate) return null
  const p = o.payload
  if (!p || typeof p !== 'object') return null
 ...
 */
function parseGameRoomReshuffle(data: unknown): GameRoomReshufflePayload | null {
  if (!data || typeof data !== 'object') return null
  const o = data as { type?: unknown; payload?: unknown }
  if (o.type !== GameRoomWs.reshuffle) return null
  const p = o.payload
  if (!p || typeof p !== 'object') return null
  const order = (p as { order?: unknown }).order
  if (!Array.isArray(order) || order.length < 1) return null
  const out: string[] = []
 ...
 */
function parseGameRoomPlayersUpdate(data: unknown): GameRoomPlayersUpdatePayload | null {
  if (!data || typeof data !== 'object') return null
  const o = data as { type?: unknown; payload?: unknown }
  if (o.type !== GameRoomWs.playersUpdate) return null
  const p = o.payload
  if (!p || typeof p !== 'object') return null
  const order = (p as { order?: unknown }).order
  if (!Array.isArray(order) || order.length < 1) return null
  ...
  if (!Array.isArray(sq)) return null
  return { order: outOrder, speakingQueue: normalizeGameRoomSpeakingQueueFlat(sq) }
}

function parseGameRoomTimerStart(data: unknown): GameRoomTimerStartPayload | null {
  if (!data || typeof data !== 'object') return null
  const o = data as { type?: unknown; payload?: unknown }
  if (o.type !== GameRoomWs.timerStart) return null
  const p = o.payload
  if (!p || typeof p !== 'object') return null
  const s = (p as { startedAt?: unknown }).startedAt
  const d = (p as { duration?: unknown }).duration
  if (typeof s !== 'number' || !Number.isFinite(s)) return null
  if (typeof d !== 'number' || !Number.isFinite(d) || d < 1000) return null
...
function parseGameRoomTimerStop(data: unknown): GameRoomTimerStopPayload | null {
  if (!data || typeof data !== 'object') return null
  const o = data as { type?: unknown; payload?: unknown }
  if (o.type !== GameRoomWs.timerStop) return null
...
function parseGameRoomPlayerKick(data: unknown): GameRoomPlayerKickPayload | null {
  if (!data || typeof data !== 'object') return null
  const o = data as { type?: unknown; payload?: unknown }
  if (o.type !== GameRoomWs.playerKick) return null
  const p = o.payload
  if (!p || typeof p !== 'object') return null
...
function parseGameRoomPlayerRevive(data: unknown): GameRoomPlayerRevivePayload | null {
  if (!data || typeof data !== 'object') return null
  const o = data as { type?: unknown; payload?: unknown }
  if (o.type !== GameRoomWs.playerRevive) return null
  const p = o.payload
  if (!p || typeof p !== 'object') return null
  const id = (p as { peerId?: unknown }).peerId
  if (typeof id !== 'string' || id.length < 1) return null
...
function parseGameRoomForceMuteAll(data: unknown): { muted: boolean } | null {
  if (!data || typeof data !== 'object') return null
  const o = data as { type?: unknown; payload?: unknown }
  if (o.type !== GameRoomWs.forceMuteAll) return null
  const p = o.payload
  if (!p || typeof p !== 'object') return null
...
function parseGameRoomPlayerLifeStateSnapshot(
  data: unknown,
): GameRoomPlayerLifeStateSnapshotPayload | null {
  if (!data || typeof data !== 'object') return null
  const o = data as { type?: unknown; payload?: unknown }
  if (o.type !== GameRoomWs.playerLifeState) return null
  const p = o.payload
  if (!p || typeof p !== 'object') return null
  const states = (p as { states?: unknown }).states
  if (!states || typeof states !== 'object' || Array.isArray(states)) return null
  const out: Record<string, GameRoomPlayerLifeState> = {}
 ...
 * snapshot request.
 */
export function useGameRoomHostSignaling(
  ...
  // never claims host.
  watch(
    [inCall, hostPeerId, hostUserId, hostSessionId, localHostSessionId, localUserId, selfPeerId, wsStatus],
    () => {
      if (!inCall.value) {
  ...
  watch(
    [reshuffleBroadcastPayload, inCall, wsStatus, isGameRoomHost],
    ([p, inCallNow, wsNow, isHostNow]) => {
      if (p == null) return
      if (!inCallNow || wsNow !== 'open' || !isHostNow) return
  ...
  watch(
    [playersUpdateBroadcastPayload, inCall, wsStatus, isGameRoomHost],
    ([p, inCallNow, wsNow, isHostNow]) => {
      if (p == null) return
      if (!inCallNow || wsNow !== 'open' || !isHostNow) return
