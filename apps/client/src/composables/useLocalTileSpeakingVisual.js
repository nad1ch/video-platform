import { getAudioAnalysisAudioContext, isAudioPlaybackUnlocked, registerAudioUnlockHook, } from 'call-core';
...
export function useLocalTileSpeakingVisual(getStream, getIsLocal, getAudioEnabled) {
    ...
    function scheduleVisual(shouldShow) {
        clearTimers();
        if (shouldShow) {
    ...
    watch(() => [getIsLocal(), getAudioEnabled(), getStream()?.id, getStream()?.getAudioTracks()[0]?.id], () => {
        if (!getIsLocal()) {
            disconnectGraph();
            return;
        }
