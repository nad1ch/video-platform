...
/**
 * Eat First host-controlled audio mix sync. Strict mirror of
 * `useMafiaAudioMixSignaling` — see that file for the full design notes.
 *
 * Direction A — host → room:
 *   The host's `setRemoteListenVolume` / `setRemoteListenMuted` calls fan out
 *   one delta entry via `eat:audio-mix-update`. The server validates host
 ...
 *   keep their personal listening prefs untouched. The OBS view **never**
 *   applies inbound `muted` for the current `hostPeerId` (snapshots may carry
 *   `muted:false`); after each batch it force-mutes the current host. On host
 *   transfer, the previous host peer is restored from `lastServerMixByPeerId`,
 *   not blindly unmuted.
 *
 ...
 */
export type EatFirstAudioMixDelta = {
  peerId: string
  volume: number
  muted: boolean
}

export type EatFirstAudioMixSignalingDeps = {
  ...
  isEatFirstHost: Ref<boolean>
  /** Server-broadcast hostPeerId from `eat:host-updated` / `eat:table-state-sync`. */
...
export type EatFirstAudioMixSignaling = {
  /**
   ...
   * when WS is closed, or when not in an Eat First call.
   */
  broadcastEatFirstAudioMixDelta: (delta: EatFirstAudioMixDelta) => void
}

function parseEatFirstAudioMixUpdate(
  data: unknown,
  ...
  if (o.type !== EatFirstWs.audioMixUpdate) {
    return null
  }
  const p = o.payload
...
export function useEatFirstAudioMixSignaling(
  deps: EatFirstAudioMixSignalingDeps,
): EatFirstAudioMixSignaling {
  const {
  ...
  /**
   * Last server-authoritative mix per peerId. Used to restore the previous
   * host's tile when host transfers — not a blind unmute.
   */
  ...
  const off = subscribeSignalingMessage((data) => {
    ...
    const parsed = parseEatFirstAudioMixUpdate(data)
    if (parsed) {
  ...
  watch(
    [isViewMode, isEatFirstRoute, hostPeerId],
    ([viewMode, eatRoute, hpid]) => {
      if (!eatRoute || !viewMode) {
  ...
  function broadcastEatFirstAudioMixDelta(delta: EatFirstAudioMixDelta): void {
    if (!isEatFirstRoute.value || !isEatFirstHost.value) {
      return
    }
    ...
    try {
      sendSignalingMessage({
        type: EatFirstWs.audioMixUpdate,
        payload: {
          entries: [
            {
              peerId,
  ...
  return { broadcastEatFirstAudioMixDelta }
}
