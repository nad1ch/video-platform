...
function parseSystemRoles(raw) {
    if (!Array.isArray(raw)) {
        return undefined;
    }
    const roles = raw.filter((role) => role === 'USER' || role === 'ADMIN' || role === 'STREAMER');
    return roles.length > 0 ? roles : undefined;
}
function parseFeaturePermissions(raw) {
    if (!Array.isArray(raw)) {
        return undefined;
    }
    const permissions = raw.filter((permission) => permission === 'EAT_FIRST_OPERATOR');
...
function parseStreamerContext(raw) {
...
function parseUser(raw) {
    ...
    const displayName = typeof u.displayName === 'string'
        ? u.displayName
    ...
    const p = u.provider;
    const provider = p === 'twitch' || p === 'google' || p === 'apple' || p === 'email' ? p : null;
    ...
    const emailVerifiedAt = typeof u.emailVerifiedAt === 'string' && u.emailVerifiedAt.length > 0 ? u.emailVerifiedAt : null;
            ...
            if (import.meta.env.DEV && next?.provider === 'twitch') {
                try {
                    ...
                    if (prev !== next.id) {
                        ...
                        authLog.info('[StreamAssist dev] Twitch user id (додай у ADMIN_TWITCH_IDS у apps/server/.env):', next.id);
                    }
                }
                catch {
                    /* private mode */
...
function logOAuthTargetWhenDevApiAmbiguous(target) {
 ...
 */
async function resetUserScopedState() {
    try {
...
export function useAuth() {
    ...
    function loginWithTwitch(redirectPath) {
    ...
    async function loginOrRegisterWithEmail(email, password, displayName) {
        const body = {
            email: email.trim(),
            password,
    ...
    async function sendEmailVerification(locale) {
    ...
    async function sendPasswordReset(email, locale) {
        const normalizedLocale = locale?.trim();
