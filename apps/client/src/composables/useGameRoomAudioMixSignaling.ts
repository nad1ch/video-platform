...
/**
 * Generic game-room host-controlled audio mix sync (Phase 3B).
 *
 * Parallel of `useMafiaAudioMixSignaling.ts`. The Mafia version is already
 * generic in shape (peerId/userId/volume/muted) — the only Mafia-coupling
 * was the wire message constant and the route-name predicate, both swapped
 * here.
 *
 * Direction A — host → room:
 *   The host's `setRemoteListenVolume` / `setRemoteListenMuted` calls fan
 *   out one delta entry via `gameroom:audio-mix-update`. The server
 *   validates host authority, stores the snapshot keyed by stable `userId`
 *   (peerId fallback), and re-broadcasts to every peer. Late joiners
 *   (notably `?mode=view` OBS) receive the full snapshot from
 *   `handleJoinRoom`.
 *
 * Direction B — room → OBS view:
 *   Only the OBS view (`isViewMode`) applies inbound entries, via the same
 *   `setRemoteListenVolume` / `setRemoteListenMuted` setters used by tile
 *   UI (call-core SSOT). The host ignores its own echoes; regular
 *   participants keep their personal listening prefs untouched. Wire
 *   `volume` is always **gain 0..2** (ParticipantTile converts slider
 *   percent 0..200 to gain via `pct/100` before `setRemoteListenVolume`).
 *   The OBS view **never** applies inbound `muted` for the current
 *   `hostPeerId` (snapshots may carry `muted:false`); after each batch it
 *   force-mutes the current host. On host transfer, the previous host
 *   peer is restored from `lastServerMixByPeerId`, not blindly unmuted.
 *
 ...
 */
export type GameRoomAudioMixDelta = {
  peerId: string
  volume: number
  muted: boolean
}

export type GameRoomAudioMixSignalingDeps = {
  ...
  isGameRoomHost: Ref<boolean>
  /** Server-broadcast hostPeerId from `gameroom:host-updated`. */
...
export type GameRoomAudioMixSignaling = {
  /**
   ...
   * when WS is closed, or when not in a game-room call.
   */
  broadcastGameRoomAudioMixDelta: (delta: GameRoomAudioMixDelta) => void
}

function parseGameRoomAudioMixUpdate(
  data: unknown,
  ...
  if (o.type !== GameRoomWs.audioMixUpdate) {
    return null
  }
  const p = o.payload
...
export function useGameRoomAudioMixSignaling(
  deps: GameRoomAudioMixSignalingDeps,
): GameRoomAudioMixSignaling {
  const {
  ...
  // Local peerId → userId index maintained from call signaling. Lets the
  // host emit entries with a stable `userId` so the server can rebind
  // across peerId changes (host or participant reload).
  const userIdByPeerId = new Map<string, string>()

  /**
   * Last server-authoritative mix per peerId (from snapshots / deltas).
   * Used to restore the previous host's tile when host transfers — not a
   * blind unmute.
   */
  ...
  const off = subscribeSignalingMessage((data) => {
    ...
    const parsed = parseGameRoomAudioMixUpdate(data)
    if (parsed) {
  ...
  // OBS view: force-mute current host playback (mic is captured separately
  // in OBS). Previous host is restored from `lastServerMixByPeerId` (not
  // blindly unmuted).
  let lastForcedHostMutePeerId: string | null = null
  watch(
    [isViewMode, isGameRoomRoute, hostPeerId],
    ([viewMode, inRoom, hpid]) => {
      if (!inRoom || !viewMode) {
  ...
  function broadcastGameRoomAudioMixDelta(delta: GameRoomAudioMixDelta): void {
    if (!isGameRoomRoute.value || !isGameRoomHost.value) {
      return
    }
  ...
  return { broadcastGameRoomAudioMixDelta }
}
