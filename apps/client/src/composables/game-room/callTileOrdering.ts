 ...
 */
export function buildHostLastOrderedTiles<T extends TileLike>(
  tiles: readonly T[],
  baseOrder: readonly string[],
  hostPeerIdForGrid: string,
): T[] {
 ...
 */
export function buildFallbackOrderedTiles<T extends TileLike>(
  tiles: readonly T[],
  tileOrder: readonly string[],
  pinnedPeerId: string | null,
  spotlightActive: boolean,
): T[] {
  const orderIndex = new Map(tileOrder.map((peerId, index) => [peerId, index] as const))
  return [...tiles].sort((a, b) => {
    if (spotlightActive && pinnedPeerId != null) {
      if (a.peerId === pinnedPeerId) return -1
      if (b.peerId === pinnedPeerId) return 1
    }
