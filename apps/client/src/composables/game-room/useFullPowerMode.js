...
export function useFullPowerMode(options) {
    const { isSystemHealthy, receiveDeviceProfile } = options;
    const isFullPowerMode = shallowRef(false);
    let fullPowerEnterTimer = null;
    function clearFullPowerEnterTimer() {
        if (fullPowerEnterTimer != null) {
            clearTimeout(fullPowerEnterTimer);
            fullPowerEnterTimer = null;
        }
    }
    watch(() => [isSystemHealthy.value, receiveDeviceProfile.value.profile === 'strong'], ([healthy, strongProfile]) => {
