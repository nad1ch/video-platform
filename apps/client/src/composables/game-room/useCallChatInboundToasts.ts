...
import { normalizeDisplayName, type CallChatLine } from 'call-core'
...
export interface CallChatInboundToast {
  toastId: string
  lineId: string
  title: string
  preview: string
...
export interface UseCallChatInboundToastsOptions {
  /**
   * Current call session "joined" flag. Falsy → composable resets the
   * queue and stops enqueuing new toasts. Page provides
   * `computed(() => session.inCall)` (or equivalent).
   */
  isInCall: ComputedRef<boolean> | Ref<boolean>
  /**
   * Mid-flight join flag from `useCallOrchestrator`. While truthy the
   * messages watcher short-circuits — toasts for messages that arrived
   * during the join would surface to a user who hasn't seen the chat
   * panel yet.
   */
  joining: Ref<boolean>
  /**
   * Route-specific OBS / view-mode gate. When truthy, the messages
   * watcher short-circuits because the toast UI is hidden in view-mode
   * anyway. Page composes this from its route-specific view flags.
   */
  isViewMode: ComputedRef<boolean> | Ref<boolean>
  /** Live chat messages from `useCallOrchestrator`. */
  callChatMessages: Ref<readonly CallChatLine[]>
  /** Self peer id from `storeToRefs(session)` (call-core defaults to a non-empty tab-scoped id). Used to skip self-toasts. */
  selfPeerId: Ref<string>
  /**
   * Writable chat-open ref from `useCallChatPanel`. The composable
   * toggles it to `true` when the user clicks a toast; the page-side
   * gesture handlers continue to drive the gestural side.
   */
  chatOpen: Ref<boolean>
}

export interface UseCallChatInboundToastsApi {
  callChatInboundToasts: Ref<CallChatInboundToast[]>
  dismissCallChatInboundToast: (toastId: string) => void
  openChatFromInboundToast: (toastId: string) => void
}

export function useCallChatInboundToasts(
  options: UseCallChatInboundToastsOptions,
): UseCallChatInboundToastsApi {
  ...
  function dismissCallChatInboundToast(toastId: string): void {
    callChatInboundToasts.value = callChatInboundToasts.value.filter(
      (x) => x.toastId !== toastId,
    )
  }
