...
import { getAnalyticsSessionId, redactSensitiveDiagnosticsText, sanitizeDiagnosticsMetadata, trackClientError, } from '@/utils/clientAnalytics';
/**
...
const PROD_FLOOR = 'warn';
const VIEW_FLOOR = 'error';
const queue = [];
let initialized = false;
let dropped = 0;
let flushTimer = null;
let levelFloor = PROD_FLOOR;
/**
...
function shouldThrottle(type, peerId) {
    if (!THROTTLED_EVENT_TYPES.has(type) || !peerId)
        return false;
    ...
    if (lastEmitByKey.has(key)) {
        ...
        if (!oldest.done)
            lastEmitByKey.delete(oldest.value);
    }
...
function computeLevelFloor() {
    if (readSearchParam('mode') === 'view')
        return VIEW_FLOOR;
    const diag = readSearchParam('diagnostics');
    if (diag === 'debug')
        return 'debug';
    if (diag === '1' || diag === 'on' || diag === 'true')
        return 'info';
    return PROD_FLOOR;
}
...
function envSnapshot() {
    ...
    if (!w)
        return undefined;
    const nav = (w.navigator ?? null);
    const out = {};
    try {
        const ua = nav?.userAgent;
        if (typeof ua === 'string' && ua.length > 0)
            out.ua = ua.slice(0, 240);
    }
    catch {
        // ignore
    }
...
function sendBatch(batch, keepalive) {
    const w = safeWindow();
    if (!w || batch.length === 0)
        return;
    let body;
...
export function setDiagnosticsContext(partial) {
    try {
        ensureInitialized();
        if ('roomId' in partial)
            context.roomId = partial.roomId ?? null;
        if ('gameType' in partial)
            context.gameType = partial.gameType ?? null;
        if ('peerId' in partial)
            context.peerId = partial.peerId ?? null;
        if ('userId' in partial)
            context.userId = partial.userId ?? null;
        if ('sessionId' in partial)
            context.sessionId = partial.sessionId ?? null;
        if ('correlationId' in partial)
            context.correlationId = partial.correlationId ?? null;
    }
    catch {
        // ignore
    }
}
export function emitDiagnosticEvent(input) {
    try {
        ensureInitialized();
        if (!isLevelEnabled(input.level))
            return;
        const ctx = {
            ...context,
            ...(input.override ?? {}),
        };
        if (shouldThrottle(input.type, ctx.peerId))
            return;
        const message = redactSensitiveDiagnosticsText(typeof input.message === 'string' && input.message.length > 0
            ? input.message
            : input.type).slice(0, 500);
...
export function getDiagnosticsDroppedCount() {
    return dropped;
}
export function flushDiagnostics(keepalive = false) {
    try {
        if (flushTimer != null) {
        ...
        while (queue.length > 0) {
            ...
            while (queue.length > 0 &&
                ...
                if (batch.length > 0 && bytes + nextBytes > BATCH_MAX_BYTES)
                    break;
                queue.shift();
                batch.push(next);
                bytes += nextBytes;
...
export function installDiagnosticsVueErrorHandler(app) {
    try {
