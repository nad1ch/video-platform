...
export const CALL_AUDIO_OUTPUT_DEVICE_ID_KEY: InjectionKey<Ref<string>> = Symbol(
  'CALL_AUDIO_OUTPUT_DEVICE_ID_KEY',
)

export async function applyCallAudioOutputSinkToStreamAudios(deviceId: string): Promise<void> {
  ...
  try {
    ...
    const setCtx = (ctx as AudioContext & { setSinkId?: (sinkId: string) => Promise<void> }).setSinkId
  ...
  for (const node of nodes) {
    const el = node as HTMLAudioElement & { setSinkId?: (sinkId: string) => Promise<void> }
