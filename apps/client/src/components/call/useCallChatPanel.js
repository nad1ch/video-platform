...
export function useCallChatPanel(input) {
    ...
    const chatPanelRect = ref(null);
    const chatPanelCustomized = ref(false);
    let chatPanelGesture = null;
    function chatOpenPrefKey() {
    ...
    function clampChatPanelRect(rect) {
        if (typeof window === 'undefined') {
            return rect;
        }
        const margin = 8;
    ...
    function onChatPanelPointerMove(ev) {
        if (!chatPanelGesture) {
            return;
        }
        ...
        const next = chatPanelGesture.kind === 'move'
    ...
    function startChatPanelGesture(ev, kind) {
        if (ev.button !== 0) {
            return;
        }
    ...
    function onChatPanelDragPointerDown(ev) {
        startChatPanelGesture(ev, 'move');
    }
    function onChatPanelResizePointerDown(ev) {
        startChatPanelGesture(ev, 'resize');
    }
    function syncChatPanelToViewport() {
    ...
    watch(() => [input.inCall(), input.roomId()], ([inCall]) => {
        if (!inCall) {
