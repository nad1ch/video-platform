...
type TileLike = { peerId: string; isLocal: boolean }

export interface UseMediaStallRecoveryDeps {
  /** Live tile list from the call orchestrator. */
  tiles: Ref<readonly TileLike[]>
  /** True when running as OBS / `?mode=view` (Mafia or Eat First). */
  isOperatingAsObsViewSource: () => boolean
  /** call-core: re-apply server producer list (soft tier). */
  requestForcedProducerResync: () => void
  /** call-core: tear down all recv consumers + fresh sync (hard tier). */
  requestHardProducerResync: () => void
}

export interface UseMediaStallRecoveryReturn {
  /** Set of peerIds currently flagged as stalling (for global health computed). */
  remotePlaybackWaitingPeerIds: Ref<Set<string>>
  /** ParticipantTile `@remote-playback-stall` handler. */
  onRemotePlaybackStall: (payload: { peerId: string; stalling: boolean }) => void
  /** ParticipantTile `@video-stall` handler. */
  onTileVideoStall: (payload: { peerId: string }) => void
  /** ParticipantTile `@audio-stall` handler. */
  onTileAudioStall: (payload: { peerId: string }) => void
}

export function useMediaStallRecovery(
  deps: UseMediaStallRecoveryDeps,
): UseMediaStallRecoveryReturn {
