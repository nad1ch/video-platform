...
import { isAudioPlaybackUnlocked, playAllPageAudioThrottled, registerAudioUnlockHook, } from 'call-core/audio-unlock';
...
import { isMediaDebugEnabled, registerAudioDebugReader, } from '@/utils/mediaDebugRuntime';
...
const el = ref(null);
const callAudioOutputDeviceId = inject(CALL_AUDIO_OUTPUT_DEVICE_ID_KEY, null);
async function applyCallOutputSinkToElement() {
...
let smoothGainRaf = 0;
let currentGain = 1;
let lastBoundAudioTrackId = null;
let lastBoundUsingWebAudio = false;
function shouldUseWebAudioPlayback() {
...
function stopSmoothGainLoop() {
    if (smoothGainRaf !== 0) {
        cancelAnimationFrame(smoothGainRaf);
        smoothGainRaf = 0;
    }
}
function stepSmoothGain() {
...
function silenceElement(detach) {
    const a = el.value;
    if (!a) {
        return;
    }
...
function softMuteElementForWebAudio(s) {
    const a = el.value;
    if (!a) {
        return;
    }
...
async function bindElementFallback(s, generation = bindGeneration) {
    const a = el.value;
    if (!a) {
        return;
    }
    ...
    try {
        ...
        const isNotAllowed = e instanceof DOMException
...
watch(() => [props.stream, props.playRev ?? 0], () => {
    void bindAudioGraph();
}, { immediate: true, flush: 'post' });
watch(() => [props.listenVolume ?? 1, props.listenMuted ?? false, props.audioProcessing], () => {
    const s = props.stream;
...
watch(() => props.listenMuted ?? false, (next, prev) => {
    ...
    requestAnimationFrame(() => {
        if (token !== listenMutedRetryToken)
            return;
        const cur = el.value;
...
let audioCtxHeartbeat = null;
function tickAudioCtxHeartbeat() {
    if (!usingWebAudio)
        return;
    if (typeof AudioContext === 'undefined')
        return;
...
let audioStallSampleTrackId = null;
let audioStallCurrentlyDetected = false;
let audioStallLastFiredAt = 0;
function shouldRunAudioStallWatchdog() {
...
function resetAudioStallWatchdog() {
...
function tickAudioStallWatchdog() {
    ...
    const track = a.srcObject.getAudioTracks()[0] ?? null;
...
watch(() => [props.audioEnabled, props.listenMuted ?? false, props.peerId ?? null], () => {
    if (!shouldRunAudioStallWatchdog()) {
        resetAudioStallWatchdog();
    }
}, { flush: 'post' });
...
watch(() => [props.audioLevel ?? 0, props.voiceDucked ?? false], () => {
    if (usingWebAudio && props.audioProcessing) {
        startSmoothGainLoop();
    }
}, { flush: 'post' });
watch(() => [callAudioOutputDeviceId?.value ?? '', el.value], () => {
 ...
 */
let detachAudioDebugReader = null;
onMounted(() => {
    ...
    if (peerId == null)
        return;
    detachAudioDebugReader = registerAudioDebugReader(peerId, () => {
        const a = el.value;
        const track = a?.srcObject ? a.srcObject.getAudioTracks()[0] ?? null : null;
        ...
        const now = typeof performance !== 'undefined' ? performance.now() : Date.now();
        const audioMutedDurationMs = audioStallSampleStartAt > 0 ? Math.round(now - audioStallSampleStartAt) : 0;
        return {
            el: a,
...
/** @type {__VLS_StyleScopedClasses['stream-audio']} */ ;
