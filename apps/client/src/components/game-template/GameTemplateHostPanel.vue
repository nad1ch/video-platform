...
/**
 * GameTemplateHostPanel — fork of `MafiaHostPanel.vue` for the
 * `/app/game-template` route. Behaviour is intentionally 1:1 with the
 * Mafia host panel: same draggable / resizable / collapsible night-action
 * grid, same role flashes, same kick/check result icons, same store
 * contract.
 *
 * The Mafia stores (`mafiaGame`, `mafiaPlayers`) and Mafia helpers
 * (`mafiaGameSeatText`, `mafiaGameTypes`) are reused on purpose — the
 * initial fork keeps the protocol on Mafia internals so the existing
 * backend continues to work end-to-end. The CSS namespace is renamed to
 * `game-template-host-panel*` so future visual changes here cannot leak
 * into production Mafia, and so a future cleanup of the Mafia version
 * cannot accidentally restyle this fork.
 */
...
<template>
  <Teleport v-if="visible" to="body">
    <button
      v-if="collapsed"
      type="button"
      class="game-template-host-panel__edge-tab sa-chip-btn game-template-host-panel__edge-tab--pulse"
      :class="{
        'sa-chip-btn--on': true,
        'game-template-host-panel__edge-tab--left': collapsedSide === 'left',
        'game-template-host-panel__edge-tab--right': collapsedSide === 'right',
      }"
      :style="tabStyle"
      ...
      @click="expandPanel"
    >
      <span class="game-template-host-panel__edge-ico" aria-hidden="true">🎭</span>
    </button>

    <aside
      v-else
      class="game-template-host-panel game-template-host-panel__shell"
      :style="panelStyle"
      :aria-label="t('mafiaPage.hostPanelTitle')"
    >
      <header
        class="game-template-host-panel__head"
        @pointerdown="onHeadPointerDown"
      >
        <span class="game-template-host-panel__grip" aria-hidden="true" />
        <h2 class="game-template-host-panel__title">{{ t('mafiaPage.hostPanelTitle') }}</h2>
        <div class="game-template-host-panel__head-actions" data-no-maid-drag>
          <button
            type="button"
            class="sa-chip-btn game-template-host-panel__collapse-action game-template-host-panel__collapse-action--clear"
            :aria-label="t('mafiaPage.clearNightActionsTitle')"
            :title="t('mafiaPage.clearNightActionsTitle')"
            @click="clearHostPanelSelections"
          >
            <svg
              class="game-template-host-panel__collapse-ico"
              viewBox="0 0 24 24"
              width="16"
              height="16"
              aria-hidden="true"
          ...
          <button
            type="button"
            class="sa-chip-btn game-template-host-panel__collapse-action game-template-host-panel__collapse-action--collapse"
            :aria-label="t('mafiaPage.hostPanelCollapse')"
            :title="t('mafiaPage.hostPanelCollapse')"
            @click="collapsePanel"
          >
            <svg
              class="game-template-host-panel__collapse-ico game-template-host-panel__collapse-ico--arrow"
              :class="`game-template-host-panel__collapse-ico--arrow-${panelCollapseSide}`"
              viewBox="0 0 24 24"
              width="16"
              height="16"
              aria-hidden="true"
      ...
      <div class="game-template-host-panel__scroller">
        <div
          class="game-template-host-panel__night-row"
          role="group"
        ...
        >
          <button
            ...
            :key="r"
            type="button"
            class="game-template-host-panel__role-col h-focus-ring"
            :class="{
              'game-template-host-panel__role-col--on': isActive(r),
              'game-template-host-panel__role-col--flash': flashRole === r,
            }"
          ...
          >
            <span class="game-template-host-panel__role-label">{{ labelForKey(r) }}</span>
            <span
              class="game-template-host-panel__role-value"
            >{{ valueText(r) }}</span>
            <Transition v-if="!oldMafiaMode" name="gthp-res" mode="out-in">
              <span
                :key="resultAnimKey(r)"
                class="game-template-host-panel__role-result"
                :class="{
                  'game-template-host-panel__role-result--check-icons': r === 'sheriff' || r === 'don',
                }"
              ...
              >
                <template v-if="r === 'sheriff' || r === 'don'">
                  <span
                    ...
                    class="game-template-host-panel__check-unknown"
                  >—</span>
                  <svg
                    v-else-if="checkResultKindForKey(r) === 'peace'"
                    class="game-template-host-panel__check-icon game-template-host-panel__check-icon--peace"
                  ...
                  <svg
                    v-else
                    class="game-template-host-panel__check-icon game-template-host-panel__check-icon--mafia"
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24"
                    fill="none"
                    aria-hidden="true"
      ...
      <div
        class="game-template-host-panel__resize"
...
.game-template-host-panel__shell {
  position: fixed;
...
.gthp-res-enter-active,
.gthp-res-leave-active {
...
.game-template-host-panel {
  
  margin: 0;
  padding: 0;
}

.game-template-host-panel__head {
  box-sizing: border-box;
  display: flex;
...
.game-template-host-panel__head:active {
  cursor: grabbing;
}

.game-template-host-panel__grip {
  display: none;
}

.game-template-host-panel__head-actions {
  display: inline-flex;
  flex: 0 0 auto;
  align-items: center;
  gap: 7px;
...
.game-template-host-panel__title {
  margin: 0;
  flex: 1 1 auto;
  min-width: 0;
...
.game-template-host-panel__collapse-action {
  flex-shrink: 0;
  box-sizing: border-box;
  width: 27px;
  height: 27px;
...
.game-template-host-panel__collapse-action:hover:not(:disabled) {
  color: #fff;
  box-shadow: none;
}

.game-template-host-panel__collapse-action--clear {
  background: rgb(169 45 47 / 0.62);
  color: rgb(255 255 255 / 0.72);
}

.game-template-host-panel__collapse-action--clear:hover:not(:disabled) {
  background: rgb(169 45 47 / 0.74);
}

.game-template-host-panel__collapse-action--collapse {
  background: rgb(74 50 116 / 0.69);
  color: rgb(255 255 255 / 0.88);
}

.game-template-host-panel__collapse-action--collapse:hover:not(:disabled) {
  background: rgb(84 57 132 / 0.78);
}

.game-template-host-panel__back-ico {
  display: block;
  width: 27px;
  height: 27px;
  pointer-events: none;
...
.game-template-host-panel__collapse-ico {
  display: block;
  flex-shrink: 0;
  pointer-events: none;
}

.game-template-host-panel__collapse-ico--arrow {
  width: 17px;
  height: 17px;
}

.game-template-host-panel__collapse-action--collapse .game-template-host-panel__collapse-ico--arrow {
  transform: translateX(-1px);
}

.game-template-host-panel__collapse-action--collapse .game-template-host-panel__collapse-ico--arrow-right {
  transform: translateX(1px);
}

.game-template-host-panel__scroller {
  flex: 1 1 auto;
  min-height: 0;
  min-width: 0;
  overflow-y: hidden;
...
.game-template-host-panel__night-row {
  display: grid;
...
.game-template-host-panel__role-col {
  box-sizing: border-box;
  display: flex;
  min-width: 0;
  flex-direction: column;
...
.game-template-host-panel__role-col:hover {
  background: rgb(84 57 132 / 0.76);
}

.game-template-host-panel__role-col--on {
  background: rgb(102 56 143 / 0.73);
  box-shadow: inset 0 0 0 1px rgb(255 255 255 / 0.12);
}

.game-template-host-panel__role-col--flash {
  background: color-mix(in srgb, var(--sa-color-primary) 20%, transparent);
}

.game-template-host-panel__role-col--flash .game-template-host-panel__role-value {
  animation: gthp-value-bump 0.38s ease;
}

.game-template-host-panel__role-label {
...
.game-template-host-panel__role-value {
  display: block;
...
.game-template-host-panel__role-col:not(.game-template-host-panel__role-col--on) .game-template-host-panel__role-value {
  color: #ffd455;
  opacity: 1;
}

.game-template-host-panel__role-result {
  display: flex;
  align-items: center;
  justify-content: center;
  text-align: center;
...
.game-template-host-panel__role-result--check-icons {
  filter: none;
  opacity: 1;
  line-height: 0;
}

.game-template-host-panel__check-unknown {
  font-size: 0.7rem;
  line-height: 1;
  opacity: 0.75;
...
.game-template-host-panel__check-icon {
  display: block;
  width: 0.86rem;
  height: 0.86rem;
  margin: 0 auto;
...
.game-template-host-panel__check-icon--peace {
  color: #22c55e;
}

.game-template-host-panel__check-icon--mafia {
  color: #ef4444;
}
...
.game-template-host-panel__resize {
  display: none;
  position: absolute;
  right: 2px;
  bottom: 2px;
...
.game-template-host-panel__resize:hover {
  opacity: 0.9;
}

.game-template-host-panel__edge-tab {
  position: fixed;
  min-width: 2.4rem;
  min-height: 2.4rem;
...
@media (prefers-reduced-motion: no-preference) {
  .game-template-host-panel__edge-tab--pulse {
    animation: game-template-host-edge-glow 2.4s ease-in-out infinite;
  }
}

@keyframes game-template-host-edge-glow {
  0%,
  100% {
    box-shadow:
