...
import { NadleWs, } from './nadleWsTypes';
import { replyJsonPingIfNeeded } from 'call-core/utils';
export function useNadleWs(options) {
    const { streamerProfile, lastError, nadleTabPeerId, log, onNewGame, afterChatLineAppended, isAuthenticated } = options;
    ...
    const ircRelayStatus = ref('idle');
    let ws = null;
    let wsTargetStreamerId = null;
    let nadleWsDisposed = false;
    let nadleWsReconnectTimer = null;
    ...
    let chatLineUid = 0;
    function clearNadleWsReconnect() {
    ...
    function nadleWsUrl(streamerId) {
        const env = import.meta.env.VITE_NADLE_WS_URL;
    ...
    watch([() => streamerProfile.value?.id ?? null, isAuthenticated], ([id, auth], oldValue) => {
    ...
    function prepareNadleWsMount() {
        nadleWsDisposed = false;
        nadleWsReconnectAttempt = 0;
    }
    function disposeNadleWs() {
