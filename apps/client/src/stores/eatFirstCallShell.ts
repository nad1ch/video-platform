...
export const useEatFirstCallShellStore = defineStore('eatFirstCallShell', () => {
   ...
   * and vice versa).
   */
  const hostInteractionMode = ref<'idle' | 'swap'>('idle')
   ...
   */
  const playersUpdateBroadcastPayload = ref<{ playerOrder: string[] } | null>(null)
   ...
   */
  const selectedTimerDurationMs = ref<number | null>(null)

  function setEatFirstSelectedTimerDurationMs(durationMs: number | null): void {
   ...
   */
  function setHostInteractionMode(mode: 'idle' | 'swap'): void {
    if (!isEatFirstRoomHost.value) return
   ...
   */
  function setHostSeatSwapSelectionPeerId(peerId: string | null): void {
    if (!isEatFirstRoomHost.value) return
   ...
   */
  function swapEatFirstSlotsInPlayerOrder(slotA: string, slotB: string): void {
    if (!isEatFirstRoomHost.value) return
  ...
  function appendSpeakingNominationPair(by: number, target: number): void {
    ...
    if (next === speakingQueue.value) return
  ...
  function applySpeakingQueueFromSignaling(seats: unknown): void {
    ...
    if (shouldClearDraft) clearSpeakingNominationDraft()
  }
