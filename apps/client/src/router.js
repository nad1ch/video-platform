...
import { loadCheckersPage, loadEatFirstPage, loadGameTemplatePage, loadMafiaPage, loadNadleStreamPage, } from '@/routerRouteLoaders';
const DEFAULT_NADLE_STREAMER = (typeof import.meta.env.VITE_DEFAULT_STREAMER === 'string' && import.meta.env.VITE_DEFAULT_STREAMER.trim()) ||
    STREAMER_NICK;
const SEO_MARKETING_SLUGS = ['video-calls-for-streamers', 'twitch-nadle-game', 'stream-overlay-tools'];
...
function seoMarketingStaticRoutes() {
...
export const router = createRouter({
    ...
    routes: [
        ...
        },
        {
            ...
            redirect: (to) => ({
                path: '/app/eat',
                query: eatGameOnlyQuery(to.query),
            }),
        },
        {
            path: '/eat/admin',
        ...
        },
        {
            ...
            redirect: (to) => ({
                path: '/app/eat',
                query: { ...eatGameOnlyQuery(to.query), mode: 'view' },
            }),
        },
        {
 ...
 */
function eatGameOnlyQuery(query) {
    const out = {};
 ...
 */
function normalizeEatLegacyQuery(to) {
    if (to.name !== 'eat')
        return null;
    ...
    const next = eatGameOnlyQuery(query);
    if (isLegacyView) {
        if (view === 'overlay') {
            next.mode = 'view';
        }
    ...
    for (const k of ['emailVerified', 'emailVerification']) {
        const v = query[k];
...
function isMafiaObsViewRoute(to) {
...
function isEatFirstObsViewRoute(to) {
    if (to.name !== 'eat') {
        return false;
    }
    const mode = to.query.mode;
...
function routeNeedsBetaAccess(to) {
    return typeof to.name === 'string' && BETA_GATED_ROUTE_NAMES.has(to.name);
}
function betaAccessModalQueryValue(to) {
    const name = typeof to.name === 'string' ? to.name : '';
    return name in BETA_ACCESS_MODAL_BY_ROUTE_NAME
        ? BETA_ACCESS_MODAL_BY_ROUTE_NAME[name]
        : 'video-call';
}
function userHasBetaAccess(user) {
    if (!user)
        return false;
...
function emailVerificationQuery(query) {
    const next = {};
...
router.beforeEach(async (to) => {
    ...
    if (isAppRoute) {
        ...
        if (isAuthenticated.value && userNeedsEmailVerification(user.value) && !isVerifyEmailRoute) {
            ...
            return {
                name: 'verify-email',
                query: emailVerificationQuery(to.query),
            };
        }
