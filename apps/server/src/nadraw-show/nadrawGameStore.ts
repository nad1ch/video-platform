...
import { clearNadrawGuessThrottleForStreamer, tryConsumeNadrawGuessThrottle } from './nadrawThrottle'
import type { GamePhase, NadrawStatePayload } from './wsProtocol'
...
export function setNadrawStateListener(fn: StateListener | null): void {
  stateListener = fn
}

export function setNadrawFeedbackListener(fn: FeedbackListener | null): void {
  feedbackListener = fn
}

export function setNadrawCanvasClearListener(fn: typeof canvasClearListener): void {
...
function fullSessionEnd(streamerId: string, room: Room): void {
  ...
  clearNadrawGuessThrottleForStreamer(streamerId)
...
async function enterBetweenRounds(streamerId: string, room: Room): Promise<void> {
  ...
  if (room.sessionWordSource !== 'manual') {
    ...
    if (!picked.ok) {
      console.error('[nadraw-show] between-rounds pick failed', picked.code)
...
function beginRoundWithWord(streamerId: string, room: Room, word: string): void {
  ...
  setStreamerActiveGame(streamerId, 'nadraw-show')
  notifyState(streamerId)
}
    ...
    try {
      ...
      if (wordSource === 'db') {
        return {
          ...
          message:
            'Could not load prompts. Apply DB migrations (drawing prompts table) and run prisma generate, then retry.',
        }
      }
...
export function getNadrawStatePayload(streamerId: string, includeSecret: boolean): NadrawStatePayload {
...
export async function hostStartNadrawRound(
  streamerId: string,
  opts: {
    wordSource: 'random' | 'db' | 'manual'
    manualWord?: string
...
export function hostClearNadrawRound(streamerId: string): void {
  const room = getOrCreateRoom(streamerId)
  clearAllTimers(room)
  room.phase = 'idle'
  room.currentWord = ''
...
export function handleNadrawChatGuess(streamerId: string, userId: string, displayName: string, text: string): void {
  ...
  if (!tryConsumeNadrawGuessThrottle(streamerId, userId)) {
